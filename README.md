...
**Model:** [ruggsea/zagreus-0.4B-italic-softkd](https://huggingface.co/ruggsea/zagreus-0.4B-italic-softkd)
...
| Stage | Wall clock | H100h |
|---|---|---|
| Ortho/morph MCQ generation (`gen_ortho_morph.py`, on-GPU) | 7 min | 0.15 |
| Committee labeling (grammar + ortho/morph + knowledge subset) | ~27 min | 0.53 |
| softKD training run (`final_train.slurm`) | 3h05m | 3.70 |
| Official full-10K eval (`gsc1_eval_only.slurm`) | 14 min | 0.28 |
| Total | | **≈4.7** |

The shipped soft-label pool is included, so a replay can also skip labeling and still stay within budget with room to spare. Evaluation uses the official `run_eval.py` from the ITALIC repo, unmodified, fast mode, 5-shot, temperature 0.

## Repository layout

- `code/` training, data-generation, labeling and eval scripts
- `data/` the final training pool with committee soft labels, plus the decontamination manifest
